...
export default function App() {
  ...
  useEffect(() => {
    const onInstall = (event: Event) => {
      ...
      setInstallEvent(event as typeof installEvent);
    };
    window.addEventListener('beforeinstallprompt', onInstall);
    navigator.serviceWorker?.addEventListener('message', (event) => {
      if (event.data?.type === 'LINKUP_UPDATE_READY') setUpdateReady(true);
    });
  ...
  return <div className="app">
    ...
    {updateReady && <div className="updatebar">A fresh LinkUp web release is ready. <button onClick={() => location.reload()}>Update now</button></div>}
    ...
    <main>
      {section === 'home' && <>
        <section className="hero">
          ...
          <div className="heroActions"><button className="primary" onClick={install}>Install LinkUp</button><button className="secondary" onClick={() => setSection('updates')}>Explore updates</button></div>
